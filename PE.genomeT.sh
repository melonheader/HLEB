#!/usr/bin/bash
### INFO: Count genome Ts from PE Slamseq
### DATE: 11.08.2020
...
echo "Experiment: $exp"
echo "Purpose: Count genome Ts in the provided .bam files from PE-Slamseq"
...
do
	...
	#first mate, fw T -
	samtools view -f 99 -b -h -L $utr_path $bam_path | \
	samtools mpileup -A -B -Q 1 -d 1000000 -f $gfa_path - | \
	awk '($3=="t")||($3=="T"){print $1,$2,$3,$4,$5,"-"}' OFS="\t" > covT.$sample_name$pileup
	#second mate, re T -
	samtools view -f 147 -b -h -L $utr_path $bam_path | \
	samtools mpileup -A -B -Q 1 -d 1000000 -f $gfa_path - | \
	awk '($3=="t")||($3=="T"){print $1,$2,$3,$4,$5,"-"}' OFS="\t" >> covT.$sample_name$pileup
	#first mate, re T +
	samtools view -f 83 -b -h -L $utr_path $bam_path | \
	samtools mpileup -A -B -Q 1 -d 1000000 -f $gfa_path - | \
	awk '($3=="t")||($3=="T"){print $1,$2,$3,$4,$5,"+"}' OFS="\t" >> covT.$sample_name$pileup
	#second mate, fw +
	samtools view -f 163 -b -h -L $utr_path $bam_path | \
	samtools mpileup -A -B -Q 1 -d 1000000 -f $gfa_path - | \
	awk '($3=="t")||($3=="T"){print $1,$2,$3,$4,$5,"+"}' OFS="\t" >> covT.$sample_name$pileup
